document.addEventListener('DOMContentLoaded', async () => {
    ...
    const loadDonations = async () => {
        ...
        try {
            ...
            result.data.forEach(doacao => {
                ...
                row.innerHTML = `<td data-label="Convidado">${doacao.convidado_nome}</td><td data-label="Tipo">${tipoFormatado}</td><td data-label="Valor (R$)">R$ ${valorFormatado}</td><td data-label="Data">${dataFormatada}</td><td data-label="Comprovante"><a href="${doacao.comprovante_url}" target="_blank" class="btn btn-secondary">Ver</a></td><td data-label="Status">${statusHtml}</td><td data-label="Ação">${actionHtml}</td>`;
    ...
    const loadCustomMessage = async () => {
        ...
        try {
            ...
            const result = await response.json();
            if(result.success) guestMessageText.value = result.data.mensagem_convidado;
